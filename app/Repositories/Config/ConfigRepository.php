...
class ConfigRepository
{
    ...
    private function addGeneralConfig($app = array()) : array
    {
        ...
        if (\Auth::user()) {
            ...
            $app['system']['date_format'] = $user->getPreference('date_format') ?? $app['system']['date_format'];
            $app['system']['time_format'] = $user->getPreference('time_format') ?? $app['system']['time_format'];
            $app['system']['locale'] = $user->getPreference('locale') ?? $app['system']['locale'];
            $app['system']['timezone'] = $user->getPreference('timezone') ?? $app['system']['timezone'];
            $app['chat']['auto_open'] = $user->getPreference('auto_open') ?? $app['chat']['auto_open'];
            $app['chat']['enter_to_submit'] = $user->getPreference('enter_to_submit') ?? $app['chat']['enter_to_submit'];
            $app['chat']['mute_sound_notification'] = $user->getPreference('mute_sound_notification') ?? $app['chat']['mute_sound_notification'];
        }

        \App::setLocale($app['system']['locale'] ?: 'en');
    ...
    public function setDefault() : void
    {
        ...
        if (\Auth::check()) {
            config([
                ...
                'config.system.locale' => \Auth::user()->getPreference('locale') ?? config('config.system.locale')
            ]);
        }
